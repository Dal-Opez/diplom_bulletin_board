...
class TestAdvertisementAPI:
    ...
    def test_user_can_create_ad(self, api_client, regular_user):
        ...
        url = reverse("advertisement-list")
        data = {"title": "New Ad", "price": 2000, "description": "Test"}
...
class TestReviewAPI:
    ...
    def test_user_can_edit_own_review(
        self, api_client, regular_user, advertisement, review
    ):
        api_client.force_authenticate(user=regular_user)
    ...
    def test_user_cant_edit_other_user_review(
        self, api_client, second_user, advertisement, review
    ):
        api_client.force_authenticate(user=second_user)
        url = reverse("review-detail", args=[advertisement.id, review.id])
        data = {"text": "Updated review"}
    ...
    def test_admin_can_edit_any_review(
        self, api_client, admin_user, advertisement, review
    ):
        api_client.force_authenticate(user=admin_user)
        url = reverse("review-detail", args=[advertisement.id, review.id])
...
class TestAdvertisementFilters:
    ...
    def setup(self, regular_user):
        Advertisement.objects.create(title="Laptop", price=1000, author=regular_user)
        Advertisement.objects.create(title="Phone", price=500, author=regular_user)
    ...
    def setup(self, regular_user):
        for i in range(5):
            Advertisement.objects.create(
                title=f"Ad {i}", price=100 + i, author=regular_user
            )
